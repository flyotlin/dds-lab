...
MyPublisher::MyPublisher(int nodeID, bool isReliable, int interval, int maxSample)
...
int main(int argc, char *argv[])
{
    if (argc != 5) {
        ...
        std::cout << "Usage: ./MyPublisher NODE_ID IS_RELIABLE INTERVAL MAX_SAMPLE" << std::endl;
        return 0;
    }
    ...
    MyPublisher *pub = new MyPublisher(nodeID, isReliable, interval, maxSample);
    if (pub->init()) {
        pub->run();
    }
